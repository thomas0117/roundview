  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (coverFile) {
      ...
      if (!uploadRes.ok) {
        ...
        setStatus({ type: 'error', message: uploadBody.error || '圖片上傳失敗' });
        return;
      }

      coverImageUrl = uploadBody.publicUrl;
    }
    ...
    try {
      ...
      setStatus({ type: 'success', message: editingId ? '文章已更新並同步到 Supabase！' : '文章已建立並同步到 Supabase！' });
      setForm(initialForm);
      setCoverFile(null);
      setEditingId(null);
      fetchPosts();
      ...
      <div className="admin-card">
        ...
        <form onSubmit={handleSubmit} className="admin-form">
          ...
          <label className="admin-label">
            ...
            />
            {coverFile && <p className="admin-hint">已選擇檔案：{coverFile.name}</p>}
            {!coverFile && form.coverImage && (
              <p className="admin-hint">
                現有封面：
                <a href={form.coverImage} target="_blank" rel="noreferrer">
                  開啟預覽
                </a>
              </p>
            )}
          ...
          <label className="admin-label">
            ...
            <input
              ...
              placeholder="預設會使用上傳後的 Supabase 公開網址"
            />
          </label>
            ...
            {posts.map((post) => (
              <article key={post.id} className="article-item admin-list-item">
                <div>
                  ...
                  {post.publishedAt && (
                    <p className="article-meta">發布時間：{new Date(post.publishedAt).toLocaleString('zh-TW')}</p>
                  )}
